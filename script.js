...
function censorBadWords(text) {
  ...
  return text.replace(regex, (match) => match[0] + '*'.repeat(match.length - 1) + match[match.length - 1]);
}
...
function showBkashPopup() {
  ...
  popup.innerHTML = `<div style="position: fixed; top: 20%; left: 10%; right: 10%; background: #222; color: white; padding: 20px; border-radius: 12px; z-index: 9999; text-align: center;">
    <h3>Support us via bKash</h3>
    <p>Donate to help improve this chatbot!</p>
    <p>bKash Number: <strong>(01829865733)</strong></p>
    <small>This will close in 2 minutes</small>
  </div>`;
